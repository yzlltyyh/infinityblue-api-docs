...
curl https://api.infinityblue.com/v1/chat/completions \
  ...
  -d '{"model":"gpt-4o","stream":true,"messages":[{"role":"user","content":"Write a haiku about streaming."}]}'
```
...
stream = client.chat.completions.create(
    model="gpt-4o", stream=True, messages=[{"role": "user", "content": "Write a haiku about streaming."}],
)
for chunk in stream:
...
```javascript
const response = await fetch("https://api.infinityblue.com/v1/chat/completions", {
  method: "POST",
